...
import rnh.mores.objects.blocks.AbstractModSignBlock;
...
@OnlyIn(Dist.CLIENT)
public class StoneSignTileEntityRenderer extends TileEntityRenderer<StoneSignTileEntity> {
   /** The ModelSign instance for use in this renderer */
   private final StoneSignTileEntityRenderer.StoneSignModel model = new StoneSignTileEntityRenderer.StoneSignModel();

   public StoneSignTileEntityRenderer(TileEntityRendererDispatcher rendererDispatcherIn) {
   ...
   public void render(StoneSignTileEntity tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn) {
      ...
      matrixStackIn.push();
      float f = 0.6666667F;
      if (blockstate.getBlock() instanceof StoneStandingSignBlock) {
         matrixStackIn.translate(0.5D, 0.5D, 0.5D);
         float f1 = -((float)(blockstate.get(StoneStandingSignBlock.ROTATION) * 360) / 16.0F);
         matrixStackIn.rotate(Vector3f.YP.rotationDegrees(f1));
         this.model.signStick.showModel = true;
      } else {
         matrixStackIn.translate(0.5D, 0.5D, 0.5D);
         float f4 = -blockstate.get(StoneWallSignBlock.FACING).getHorizontalAngle();
   ...
   public static Material getMaterial(Block blockIn) {
      StoneSignType stoneType;
      if (blockIn instanceof AbstractSignBlock) {
    	  stoneType = ((AbstractModSignBlock)blockIn).getStoneType();
      } else {
    	  stoneType = StoneSignType.STONE;
      }

      return ModAtlases.getSignMaterial(stoneType);
   }

   @OnlyIn(Dist.CLIENT)
   public static final class StoneSignModel extends Model {
      public final ModelRenderer signBoard = new ModelRenderer(64, 32, 0, 0);
      public final ModelRenderer signStick;

      public StoneSignModel() {
